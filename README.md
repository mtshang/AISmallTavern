...
- **RAG 长期记忆**：本地嵌入模型（granite-embedding-311m-multilingual-r2，512 维）+ FAISS 余弦检索。世界观 Markdown 按标题分块、向量化并持久化，源文件变更后自动检测哈希并重建索引；检索阈值经正负样本校准（有答案查询 top1 分布 0.865~0.942，无答案 0.760~0.844，取交界 0.85，详见 `eval/检索质量测评报告.md`），无关问题零注入。
...
本项目在 `eval/` 目录维护离线测评套件，按大类分为三个子目录；每类测评的完整数据、逐条明细与结论整理为一份中文报告，统一存放在 `eval/` 根目录（`eval/README.md` 内有运行方式与目录说明）：

| 报告 | 内容简述 |
| --- | --- |
| [`eval/检索质量测评报告.md`](eval/检索质量测评报告.md) | 正/负/边界样本分数分布、阈值扫描（MIN_SCORE=0.85 的校准依据）、SCORE_GAP 扫描、热检索延迟分解与逐条明细 |
| [`eval/工具调用测评报告.md`](eval/工具调用测评报告.md) | 工具分发层契约、Mock 工具循环（分片参数合并 / 重试与请求上限 / 异常兜底）、真实模型 40 条意图样本 × 3 次采样的工具决策评测 |
| [`eval/端到端问答测评报告.md`](eval/端到端问答测评报告.md) | 封闭式硬事实问题集上的 A/B 对比：无 RAG 注入 vs 有 RAG 注入的答对率差异 |

检索与工具循环类测评零成本、纯本地可复现；真实调用类测评使用 `.env` 中配置的模型服务（费用由服务商决定）。
